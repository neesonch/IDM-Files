...
function clickMove (t) {
	if (!win) {
		if (t.src.indexOf("blank_tile") !== -1 && turn % 2 === 0) {
		t.src = "images/nought.png";
		board[t.id[1]][t.id[2]]= player1;
		//alert (board[t.id[1]][t.id[2]])
		win = checkWin(board,player1);
		turn += 1;
		}
		else if (t.src.indexOf("blank_tile") !== -1 && turn % 2 === 1) {
		t.src = "images/cross.png";
		board[t.id[1]][t.id[2]]= player2;
		//alert (board[t.id[1]][t.id[2]])
		win = checkWin(board,player2);
		turn += 1;
		}
		else {
		alert ('Location already taken!');
		}
	}
	if (win) {
		alert("Game is over!");
